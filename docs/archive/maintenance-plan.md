...
> ## ★ 2026-08-31 — **아카이브. 역할이 끝났다**
>
> P0·P1 은 전부 닫혔고(아래 진행 상황 표가 커밋까지 들고 있다), **남아 있던 것은
> `docs/issues/` 로 한 건씩 옮겼다.** 계획 문서는 항목이 닫혀도 문서 전체가 남아서
> 무엇이 살아 있는지 읽어서 판별해야 한다 — `known-issues.md` 를 쪼갠 것과 같은 이유다.
>
> | 남아 있던 것 | 어디로 |
> |---|---|
...
> | 무엇 | 왜 안 옮겼나 |
> |---|---|
> | LICENSE · NOTICE · 보안 신고 경로 (§E-5) | **공개를 안 하기로 했다** (2026-08-31). 공개하기로 하면 이 문서를 다시 연다 |
> | 전체 ref secret scan · `git filter-repo` (§E-1) | 같음. 노출된 키는 **무효 확인됨** — 죽은 문자열이 비공개 이력에 남는 것은 감수한다 |
> | 타사 스크린샷 재배포 (§E-2) | 같음. 재배포가 문제이지 보관이 문제가 아니다 |
> | 원격 죽은 브랜치 3개 (§F · P3) | ✅ **2026-08-31 정리했다** (`docs-refactor`·`life-trainer-web-agent`·`qwen3-openclaw-agent`) |
> | `bench/` ↔ `measure/` 이름 통일 (§F · P3) | [`docs/folder-structure.md`](../folder-structure.md) 가 담당한다 — 폴더 재구조화의 일부다 |
> | 죽은 DKMS 빌드 2개 | 해롭지 않고 디스크 몇 MB 다. 커널을 올릴 때 같이 정리한다 |
> | 커밋 메시지 언어 통일 | 공개 전 항목이었다. 지금은 영어로 굳었다 |
>
> `h-0007`·`h-0008`~`h-0010` 사슬은 **이미 `issues/` 에 있었다.**
> 아래 본문은 **그날의 판단 기록**이다. 오늘 상태와 어긋나는 곳이 있으면 오늘이 맞다.
...
| 그래서 **키를 회전해야 한다** | **✏️ 정정 — 이미 회전됐다** | 이력의 키와 현재 키의 sha256 이 다르다. [known-issues §17](../../life-trainer/docs/known-issues.md) 이 08-27 재발급 + 옛 키 403 확인을 기록. **남은 것은 공개 이력의 죽은 문자열뿐** |
...
| 파이썬 의존성이 하한만 있고 lock 이 없다 | **✅ 확인** | [pyproject.toml](../../life-trainer/pyproject.toml) 전부 `>=` |
...
> ([CLAUDE.md](../../life-trainer/CLAUDE.md)). 리뷰도 조사 문서다. 재현되는 것만 계획에 넣는다.
...
**D 를 A 와 같이 둔다.** [handbook](../../life-trainer/docs/handbook.md) 이 *"Serper 키가 없다"* 고
...
| 출력 | 실제 | 원인 ([measure/tools/verify-jetpack.sh](../../measure/tools/verify-jetpack.sh)) |
|---|---|---|
...
- [ ] Super Mode 안내를 **조건부로** 바꾼다. 이 보드에서 안 되는 것이 이미 확인됐으므로,
      절차를 노출하지 말고 *"이 장비에서는 불가 — 근거는 `measure/findings/hardware.md`"* 로
...
[docs/handbook.md](../../life-trainer/docs/handbook.md) 헤더가 `기준일: 2026-08-28` 인데 본문 7곳이
그 이전을 말한다. **아래는 파일에 박혀 있는 고정 문자열이다** — 살아 있는 값이 아니라 증거다.

| 줄 | 문서에 박혀 있는 것 | 실제 |
|---|---|---|
| `:334` `:477` | 웹 검색 "한국어=**네이버** / 그 외=Serper · **키 대기**" | 네이버 경로는 08-25 에 **코드에서 삭제**됐다. [`llm/websearch.py`](../../life-trainer/lifetrainer/llm/websearch.py) 가 첫머리에 그렇게 적고 있다. 키는 08-21 투입 |
...
같은 병이 [HANDOFF §4](../../life-trainer/HANDOFF.md) 에 남아 있다 — §1 은 08-28 에 명령으로 바꿨는데
**§4 는 숫자 블록 그대로다.**

**원인은 문서가 두 시제를 섞고 있다는 것이다.** handbook 은 "무엇이 있고 어떻게 흐르는가"(구조,
거의 안 변함)와 "지금 무엇이 막혀 있는가"(현황, 매일 변함)를 한 파일에 담는다. 후자는 이미
[`docs/issues/`](../../life-trainer/docs/issues) 와 `lt doctor` 가 하는 일이고,
[issues/README](../../life-trainer/docs/issues/README.md) 가 상태 열을 안 두는 이유와 정확히 같다.
...
- [ ] [docs/README.md](../README.md) `:24` 의 `openclaw-setup/` → `operate/` (08-28 해체된 옛 이름)
...
├── shellcheck bench/*.sh life-trainer/scripts/*.sh   (신규)
└── systemd-analyze verify  유닛 정적 검사 (신규)

make check-fast     ← pre-push 용. links + check-docs 만
```

- [ ] `operate/tools/check-docs.sh` — 금지 대상을 **숫자 일반이 아니라 "운영 지표 리터럴" 로 한정**한다
      (이벤트 수·문서 수·테스트 수·잡 수·규칙 수). 예외는 **한 목록에** 둔다
- [ ] 예외 경로는 [check-links.sh](../../operate/tools/check-links.sh) 가 이미 쓰는 것을 재사용한다 —
...
- [HANDOFF §10 빠른 확인](../../life-trainer/HANDOFF.md) 이 `journalctl --user -u lifetrainer-sync -n 20`
...
- [ ] [verify-boot.sh](../../operate/tools/verify-boot.sh) 에 "journal 이 이전 부팅을 갖고 있나" 한 줄 추가
...
[`lt backup`](../../life-trainer/lifetrainer/cli.py) 은 SQLite 온라인 백업 API 를 쓴다. **구현은 옳다.**
없는 것은 그 주변 전부다:
...
[operate/README](../../operate/README.md) 가 08-28 콜드 부팅 검증을 기록했다. 그건
**"지금 것이 다시 뜨는가"** 를 확인한 것이지 **"처음부터 세울 수 있는가"** 가 아니다. 실측:

| | [install-units.sh](../../life-trainer/scripts/install-units.sh) | [uninstall-units.sh](../../life-trainer/scripts/uninstall-units.sh) | 지금 |
|---|---|---|---|
...
1. **install 이 안 켜는 것을 uninstall 도 안 끈다** — 설치→제거를 돌려도 지금 도는 6개 중
   셋(web · llama-embed · slack)이 남는다. *"자동화 전체 끄기"* ([HANDOFF §9](../../life-trainer/HANDOFF.md))
   가 실제로는 전체를 안 끈다
...
- [ ] [verify-boot.sh](../../operate/tools/verify-boot.sh) 에 **desired-state 대조**를 추가한다 —
      지금은 "떠 있나"만 본다
...
[HANDOFF §4](../../life-trainer/HANDOFF.md) 가 08-24부터 *"203건이 08-21 수정 이전 잔재인지 안 갈렸다"*
...
- [ ] [`deploy/use-system-node.sh`](../../life-trainer/deploy/use-system-node.sh) 적용
...
- [ ] **`make host-status`** (또는 `operate/tools/host-status.sh`) 신설:
...
- [ ] [operate/systemd/llama-server.service](../../operate/systemd/llama-server.service) 의
...
[known-issues §17](../../life-trainer/docs/known-issues.md) 이 08-27 재발급과 옛 키 폐기 확인(HTTP 403)을
...
- [ ] `git filter-repo` 로 `life-trainer/config/*.bak-*` blob 제거 여부 결정.
...
[design/refs/10min_design/](../../life-trainer/docs/design/refs/10min_design/README.md) 에 타사 제품
캡처 17장(약 5MB)이 있다. 그 README 는 **상표 주의를 훌륭히 적어 뒀지만** 그건 *이름을 안 쓴다*
는 판단이고, **이미지 재배포는 별개 문제**다.

이 저장소는 같은 종류의 판단을 이미 두 번 내렸다 — `refs/reference/`(클론 → URL+커밋만),
`refs/models/`(가중치 → 선택 근거만). **같은 원칙을 여기에도 적용한다.**
...
또 [thermal-test.sh](../../measure/tools/thermal-test.sh) `:74` 가 **92°C 를 "스로틀링 영역"** 이라 적는다.
...
- [ ] 루트 [README](../../README.md) 에 **"1회 측정"** 명시 — [measure/README](../../measure/README.md) 는
      이미 적고 있는데 루트에는 없다

## E-4. 의존성·업그레이드 정책 ✅

[pyproject.toml](../../life-trainer/pyproject.toml) 의 의존성이 전부 하한(`>=`)이고 lock 이 없다.
...
| **`bench/` ↔ `measure/` 이름 통일** | 링크 churn 이 크고, 미루는 동안 위험이 안 는다. **리뷰의 P3 판정에 동의한다** |
...
[CLAUDE.md](../../life-trainer/CLAUDE.md) 에 한 줄 남긴다. **과거 커밋은 고치지 않는다.**
...
life-trainer/.venv/bin/lt doctor              # 17항목
...
ls -la life-trainer/data/backup/              # §B-1
