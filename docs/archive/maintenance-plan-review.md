...
> 거기서 살아남은 항목은 다시 [`docs/issues/`](../../life-trainer/docs/issues/) 로 갔다.
...
[`measure/tools/verify-jetpack.sh`](../../measure/tools/verify-jetpack.sh)가 다음 항목을 잘못 판정한다.

### L4T 버전이 빈칸으로 나온다

[`verify-jetpack.sh:21`](../../measure/tools/verify-jetpack.sh)의 정규식이 실제
...
[`verify-jetpack.sh:32`](../../measure/tools/verify-jetpack.sh)는 심볼릭 링크인
...
[`verify-jetpack.sh:48`](../../measure/tools/verify-jetpack.sh)은 `/dev/nvhost-nvdla*`만 찾는다.
현재 기기에는 다음 항목이 존재한다.
...
[`verify-jetpack.sh:91`](../../measure/tools/verify-jetpack.sh)은 conf 심링크를 바꾸고 Super Mode를
...
[`lt backup`](../../life-trainer/lifetrainer/cli.py)은 SQLite 온라인 백업 API를 사용하므로
...
현재 백업은 원본 DB와 같은 NVMe의 `life-trainer/data/backup/`에만 있다. 이 사본은
...
[`life-trainer/.gitignore`](../../life-trainer/.gitignore)는 실제 Serper API 키가 포함된 설정
...
야간 종료 경로에는 이미 [`embed_pending`](../../life-trainer/lifetrainer/cli.py)이 연결돼 있다.
...
현재 [`life-trainer/scripts/install-units.sh`](../../life-trainer/scripts/install-units.sh)은
...
반대로 [`uninstall-units.sh`](../../life-trainer/scripts/uninstall-units.sh)은 다음을 누락한다.
...
[`operate/systemd/llama-server.service`](../../operate/systemd/llama-server.service)의 readiness 처리도
...
[`measure/tools/thermal-test.sh`](../../measure/tools/thermal-test.sh)의 5분 실행은 빠른 냉각 이상 탐지에는 유용하다.
...
- [ ] `bench/`와 `measure/` 폴더 구조 변경 여부 결정
