...
**측정 결과 자체는 여기 없다.** 그건 [`measure/findings/`](../measure/findings/) 에 있다.
...
| 폴더 | 뜻 | 다시 읽을 이유 |
|---|---|---|
| **[../CLAUDE.md](../CLAUDE.md)** | 저장소 전체 **작업 규칙** | 검사·경보를 붙이기 전 · 되돌릴 수 없는 것을 건드리기 전 |
| **[../operate/](../operate/)** | 지금 이 기기에서 **돌고 있는 것**의 구축 기록 | 재현·복구·함정 회피 |
...
| 문서 | 무엇 | 상태 |
|---|---|---|
| [operate/notes/llm-runtime.md](../operate/notes/llm-runtime.md) | llama.cpp CUDA 빌드, llama-server 운영, 모델 로드 | ✅ `llama-server.service` active |
| [operate/notes/agent-gateway.md](../operate/notes/agent-gateway.md) | OpenClaw 게이트웨이 + 로컬 LLM 결합, **함정 14가지** | ✅ 게이트웨이 active (Slack 채널만 꺼짐) |

실행 자산(유닛·스크립트)은 문서가 아니라 [`operate/`](../operate/) 에 있다.

> **`operate/notes/agent-gateway.md §4` 는 코드에서 직접 참조된다.** `llm/client.py`,
...
| 문서 | 무엇 | 어디까지 |
|---|---|---|
| [plans/opensource-plan.md](plans/opensource-plan.md) | 측정 자료 공개·`whichllm` 기여 | 개인정보·Git 이력 정리 완료, **GitHub 게시 준비 중** · 외부 PR 승인 대기 |
...
세 갈래 중 살아남아 상시 구동에 들어간 것은 **[Life Trainer](../life-trainer/)** 다.
자체 문서 트리를 갖고 있다 — [HANDOFF.md](../life-trainer/HANDOFF.md) 부터 읽는다.
