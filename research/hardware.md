...
> ⚠️ **2026-08-18 정정.** 아래 60 GB/s 를 한때 "하드웨어 천장" 으로 읽었으나
> 모델 13종 실측이 반증했다 — **단일 커널 마이크로벤치의 한계**다.
> 자세한 것은 [performance.md §2](performance.md).

```
사양치 (LPDDR5 128-bit)      102.4 GB/s
─────────────────────────────────────────
단일 커널 순차 읽기           60.0 GB/s     ← 마이크로벤치 한계
D2D 복사 (읽기+쓰기)         69.5 GB/s
실제 달성 최대 (8B Q8_0)     86.4 GB/s     ← 사양의 84%
```
