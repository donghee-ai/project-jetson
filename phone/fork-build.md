...
> 작성 2026-08-21.
>
> ★ 원래 여기 `research.md`·`plan.md`·`runbook.md` 링크가 있었는데 **그 세 파일은
> 어디에도 없다** (2026-09-05 전수 확인). 만들다 만 것인지 지워진 것인지 모른다.
> 필요한 규격은 위에 적은 대로 **이 문서 안에 전부 옮겨져 있으므로** 링크만 지웠다.
...
| 이벤트 `data` | `app` = **앱 이름**("카카오톡") / `package` = 패키지명 / `classname` | master `models/Event.kt` |
...
한다.** 그건 나중에 해도 된다 — 실제로 나중에 했고, 결과는
[STATUS.md](STATUS.md) 의 "터널" 항목에 있다 (`cloudflared-lt.service`, `/ingest/` 만 공개).
