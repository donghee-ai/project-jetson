...
이 프로젝트가 실제로 밟은 지뢰 세 개를 여기서 막는다(`docs/openclaw-agent.md §4`,
...
def _check_no_pattern(node: Any, *, _path: str = "$") -> None:
    ...
    거부한다 (실측: `docs/openclaw-agent.md §4-5`). `properties`/`$defs`/`items`/
...
class LLMClient:
    ...
    def model_id(self) -> str | None:
        ...
        `docs/openclaw-agent.md §4-8`). 호출부는 헬스체크 통과만 믿지 말고
