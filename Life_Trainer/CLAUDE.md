...
- 활동 카테고리 **9개** (게임 포함) + 구조 상태 3개 · 분류 규칙 **40개** (앱 34 · URL 6 · 제목 4)
...
```bash
.venv/bin/python -m pytest tests/ -q     # 기준선 1136개 (2026-08-24). 아래로 내려가면 안 된다
.venv/bin/lt doctor                       # 17항목 점검 (임베딩 · 에이전트 포함)
```

**기준선 숫자를 고칠 때는 실제로 돌려서 나온 값을 쓴다.** 이 저장소는 한때 네 문서가
서로 다른 테스트 수를 말하고 있었다 (858 · 826 · 687 · 584). 2026-08-24 에 같은 일이
분류 규칙 수(47 · 56 · 66)에서 또 나왔다 — **숫자를 적기 전에
[HANDOFF §10](HANDOFF.md#10-빠른-확인) 의 명령으로 뽑는다.**
