...
def cmd_doctor(args: argparse.Namespace, cfg: Config) -> int:
    ...
    ★ **판정은 `lifetrainer/health.py` 가 한다.** 여기는 찍고 exit code 만 정한다 —
      2026-09-07 에 507줄짜리 한 함수에서 갈랐다. 이유는 그 파일 머리말에.
    """
    from lifetrainer import health

    print(f"Life Trainer doctor — root={cfg.root} tz={cfg.timezone}")
    print()

    report = health.run_all(cfg)
    for status, name, detail in report.checks:
        print(f"[{status:>4}] {name:<14} {detail}")

    oks = len(report.checks) - len(report.fails) - len(report.warns)
    print()
    print(f"총 {len(report.checks)}개 항목 — OK {oks} / WARN {len(report.warns)} / FAIL {len(report.fails)}")

    if report.fails:
        print()
        print("조치가 필요합니다:")
        for _, name, detail in report.fails:
            print(f"  - {name}: {detail}")
        return 1
