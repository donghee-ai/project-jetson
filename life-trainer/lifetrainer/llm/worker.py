...
def summarize_doc(
    ...
    # ★ "한국어로"만 적었더니 8B 가 **원문 언어를 따라갔다** — 중국어 글은 중국어로,
    #   영어 글은 영어로 요약해 다이제스트에 섞여 나왔다. 원문 언어와 무관함을 못박는다.
    system = (
        "당신은 문서를 한국어로 간결하게 요약하는 도우미입니다.\n"
        "★ 원문이 영어·중국어·일본어 무엇이든 **요약은 반드시 한국어로** 씁니다. "
        "원문 언어를 따라가지 마세요.\n"
        "3~4문장으로 요약하고 핵심 키워드 태그를 6개 이하로 뽑으세요. "
        "제목을 그대로 옮겨 적지 말고 무엇을 다루는 글인지 쓰세요."
    )
    # ★ 언어 지시를 **본문 뒤**에 한 번 더 놓는다. 시스템에만 적었더니 8B 가 원문
    #   언어를 그대로 따라갔다(중국어 글 → 중국어 요약). 소형 모델은 가까운 지시를
    #   더 세게 듣는다 — 프롬프트로 안 되면 구조로, 구조 전에 자리부터 바꿔 본다.
    user = (
        f"제목: {title}\n\n본문/초록:\n{abstract}\n\n"
        "---\n위 글이 어떤 언어로 쓰였든 **summary 는 한국어 문장으로** 작성한다."
    )
