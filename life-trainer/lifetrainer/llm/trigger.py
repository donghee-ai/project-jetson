...
같은 구조를 음성 쪽에서 이미 실측했다 (`docs/archive/speech/voice-agent-plan.md §2-5`
— 음성 트랙은 접었지만 **이 결론은 여기로 옮겨와 돌고 있다**):
...
(`operate/notes/agent-gateway.md §3`)이므로 스키마 토큰은 그대로 지연이 된다.
...
## 왜 관대하게 잡는가 — 비용이 비대칭이다
    ...
    누락(FN)        "내일 3시에 알려줘" 에 툴이 없으면 모델이 "알겠습니다" 로
                    ...
                    (`operate/notes/agent-gateway.md §4-6` 의 실패 모드).
...
_PERSONAL_PATTERNS = [
    ...
    # ★ "오늘은?" 같은 **맨몸 시간낱말 되묻기.** 위의 `\S+(은|는)\?` 되묻기 패턴에
    #   걸려 웹이 켜졌고, 직전 질문의 낱말이 앞에 붙어 개인 발화가 검색어로 나갔다
    #   (`docs/issues/0001`). "오늘자 뉴스 뭐 있어?" 는 뒤에 말이 더 있으므로 안 걸린다 —
    #   **문장 전체가 시간낱말 하나뿐일 때만** 뺀다.
    re.compile(r"^\s*(오늘|어제|그제|그저께|내일|모레|이번\s*주|지난\s*주|이번주|지난주)"
               r"\s*(은|는|이|가)?\s*\??\s*$"),
]
...
def is_personal(text: str) -> bool:
    ...
    바깥으로 내보내면 안 되는 문자열을 가르는 판정이다. 툴 선택(`select_tools`)과
    발송 직전 차단(`llm/websearch.search`)이 **같은 기준을 쓰게** 하려고 공개한다 —
    그 둘이 서로 다른 문자열을 보고 있었던 것이 `docs/issues/0001` 의 원인이었다.
    """
    return any(p.search(text or "") for p in _PERSONAL_PATTERNS)
