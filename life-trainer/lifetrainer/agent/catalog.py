...
`operate/notes/agent-gateway.md §3` 의 실측: 프레임워크 기본 구성의 시스템 프롬프트가
...
#
# ① **지연.** 프롬프트 처리 295 tok/s (`operate/notes/agent-gateway.md §3`). 첫 호출 프롬프트를
...
def _slash_description() -> str:
    ...
    return (
        ...
        # ★ 실측: 계획을 **번호로** 지목하면 3/3 틀렸다 — 목록의 엉뚱한 번호를
        #   고르거나(`/done 90`), 번호가 없으면 지어냈다(`/done 1`).
        #   제목으로 지목하면 정확하다. 사용자가 말한 그 낱말이라 옮길 일이 없다.
        "★ 계획은 **제목**으로 지목한다 (`/done 미팅`).\n명령표:\n" + "\n".join(lines)
    )
    ...
    AgentTool(
        ...
        schema={
            ...
            "properties": {
                ...
                # ★★★ **enum 인 이유** (2026-08-24, 3단계 중 2단계)
                #
                #   자유 문자열이던 시절 모델이 `"today"` 를 보내 **어제 목록을 보던
                #   맥락에서 오늘 것을 바꿨다.** 시스템 프롬프트의
                #   `## Current Date & Time` 이 대화 맥락보다 센 신호이고, 무엇보다
                #   **"직전과 같은 날"을 말할 방법이 스키마에 없었다.**
                #
                #   enum 은 llama.cpp 가 GBNF 로 강제한다 — 이 다섯 중 하나만 나온다.
                #   `day` 를 필수로 만들었을 때 누락이 3/3 → 0/3 이 된 것과 같은 수법인데,
                #   이번에는 **선택지에 "이어가기"를 넣어 준다.** 표현할 말이 없으면
                #   모델은 기본값으로 떨어진다.
                # ★★ **enum 을 풀었다** (실측 뒤).
                #
                #   enum 으로 묶었을 때, 같은 턴 안에서 두 툴이 다른 날짜를 썼다:
                #       get_plans → {"day": "2026-08-23"}      ← 어제. 정확
                #       slash     → {"day": "오늘"}            ← 틀림
                #   "어제 저녁약속이랑 미팅 완료로 표시해줘" 처럼 **사용자가 어제라고
                #   말해도** 그랬다. `get_plans` 는 날짜를 직접 쓸 수 있어서 맞혔고,
                #   `slash` 는 낱말 중에서만 고를 수 있어 기본값으로 떨어진 것이다.
                #
                #   enum 을 넣은 이유는 하루 경계(06:00) 때문이었다 — 00:00~06:00 에
                #   모델이 날짜를 계산하면 이틀 뒤가 나온다. 그런데 그 위험은 하루
                #   6시간이고, 대가는 **하루 종일 쓰기가 엉뚱한 날로 가는 것**이었다.
                #   교환이 잘못됐다.
                #
                #   `_resolve_day` 가 낱말과 YYYY-MM-DD 를 둘 다 푼다. 두 툴이 같은
                #   모양이 되는 것이 덤이다 — 한 툴에서 되는 표현이 옆 툴에서 안 되면
                #   모델이 헷갈린다.
                "day": {
                    "type": "string",
                    "description": (
                        "사용자가 말한 날. YYYY-MM-DD 또는 오늘/어제/내일/모레. "
                        "직전 턴에서 보던 날을 이어가면 '직전과 같은 날'."
                    ),
                },
            },
...
def assert_no_pattern() -> None:
    ...
    요청 전체가 400 이 됐다 (`operate/notes/agent-gateway.md §4-5`).
    """
