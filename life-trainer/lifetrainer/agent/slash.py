...
**모델이 "했습니다"로 끝내는 실패 모드**(`operate/notes/agent-gateway.md §4-6`)가 되살아난다.

★ **정확히 말하면 — `slash` 자신도 MCP 툴 하나다.** 모델이 그 툴을 아예 안 부르고
  "했습니다" 로 끝내는 것은 **여전히 가능하다.** 툴을 얇게 만든다고 그 실패가 사라지지 않는다.

  이 설계가 실제로 막는 것은 셋이다:

    ① **부르고 나서 다른 일을 하는 것.** 인자가 명령 문자열 하나뿐이고 파서가 엄격해서,
       모델이 "부르긴 했는데 엉뚱한 대상에" 는 안 된다 (`/done` 을 번호로 지목하면
       3/3 틀렸던 실측이 그래서 제목 지목으로 바뀌었다)
    ② **결과가 안 보이는 것.** 반환문에 **결과 상태**를 담는다 — "껐습니다" 가 아니라
       `프라이빗: 꺼짐`. 요청과 결과가 어긋나면 사람 눈에 보인다
    ③ **사람이 직접 칠 때는 모델이 아예 없다.** Slack 슬래시는 파싱 → SQL 이라
       그 경로에는 모델 실패 모드가 존재하지 않는다
...
COMMANDS: dict[str, str] = {
    ...
    #   불가능하지는 않다(툴을 아예 안 부를 수 있다) — 다만 **부르고 나서 다른 일을
    #   하는 것**은 막히고, 반환문이 결과 상태를 말하므로 어긋남이 눈에 보인다.
    #   다른 명령이면 답이 틀리고 말지만, 프라이빗은
...
def _day_note(name: str, day: str | None) -> str:
    ...
    ★ **이 줄로는 못 고쳤다.** 기록을 위해 순서를 적어 둔다:

        프롬프트에 "날짜를 주라"고 씀      누락 1/2 → 3/3 (악화)
        결과에 이 경고를 붙임              누락 3/3 (그대로)
        스키마에서 `day` 를 **필수로**      누락 0/3 · 왕복도 하나 줄었다

    소형 모델에게 **선택 인자는 없는 것과 같다.** 문장으로 부탁하는 것과 구조로
    강제하는 것의 차이가 여기서 갈렸다 — 이 저장소가 반복해서 배운 그것이다.

    그래도 남겨 두는 이유: MCP 를 안 거치는 호출부(`lt agent call`, 파이썬 직접
    호출)는 여전히 날짜를 빠뜨릴 수 있고, 그때 조용히 오늘에 들어가면 안 된다.
    """
