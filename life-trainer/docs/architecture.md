...
>
> 스키마는 테이블 30(일반 29 + FTS5 1) + 뷰 1. **모듈·줄·테스트 수는 여기 적지 않는다** —
...
일반 테이블 29개를 한 장에 그리면 못 읽는다. **계층 셋 + 부속**으로 나눈다.
...
| 판정 | 왜 |
|---|---|
| **검색은 질의어만 내보낸다** | 이 앱의 전제가 "데이터가 기기 밖으로 안 나간다"이고 검색이 유일한 예외다. 120자에서 자른다 |
