...
5. **감시를 붙일 때 "언제 안 울리나"를 안 정했다** — 2026-08-28 하루에 다섯 번 밟았다.
   내용은 [저장소 규칙 §1](../CLAUDE.md) 에 있다 (여기 옮겨 적지 않는다 — 2번이 그 규칙이다)
...
- 활동 카테고리 **9개** (게임·SNS 포함) + 구조 상태 **자리비움·꺼짐·미분류·프라이빗·수면**.
  ★ 세지 말고 이름으로 적는다 — `3개` 라고 적어 뒀다가 프라이빗(09-01)·수면(09-05)이
  들어오면서 두 번 어긋났다. 예약 카테고리 목록은 `config/rules.yaml` 이 정본이다.
...
- **검색은 질의어만 내보낸다.** 120자에서 자르고, 나가기 직전에 `is_personal` 로 한 번 더
  본다. 다만 그 판정은 **자기지칭 어형**을 보는 것이라, 모델이 컨텍스트의 제목을 낱말로
  뽑아 검색어에 넣는 것까지는 못 막는다 — 알려진 한계다 (`docs/issues/0031`)
...
| **4 (08-28 오후)** | **테스트 수 · 실데이터** | 한 곳으로 줄인 지 **두 커밋 만에** 또 어긋남 |

**규칙이 부족한 게 아니라 숫자를 적을 자리가 여럿인 것이 원인이다.** 세 번째에
적을 자리를 하나로 줄였는데 — **그것도 부족했다.** 네 번째가 두 커밋 만에 났다.

**옮겨 적는 행위 자체가 원인이다.** 네 번째에 명령으로 대체했다:

```bash
make status    # 서비스 · 실데이터 · schema · 분류 규칙 · 링크
make test      # 테스트 개수
```

`HANDOFF §1` 은 이제 숫자 대신 이 명령을 가리킨다. 날짜 붙은 스냅숏만 참고로 둔다.
...
make hooks         # pre-push 훅 설치 — 기기마다 한 번
```

- 막는 것은 **운영 지표**(이벤트·문서·요약·테스트·잡·규칙·링크 수)뿐이다.
  토큰 수·메모리·속도 같은 **잰 시점이 있는 측정값은 안 막는다** — 그건 사실이다
