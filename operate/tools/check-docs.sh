...
PATTERNS=(
  ...
  # ★ 2026-08-31: `passed` 가 붙은 형태만 봤더니 세 곳을 놓쳤다 —
  #   `pytest tests/ -q  # 1,136개` 처럼 **주석**으로 적은 것,
  #   `tests/  47 파일 · 15,863줄 · 1,136개` 처럼 트리 그림 안에 적은 것.
  #   셋 다 실제로 낡아 있었다 (실측 1,186).
  '테스트[^\n]*[0-9],[0-9]{3}|[0-9],[0-9]{3}(개)? 테스트|pytest[^\n]*[0-9],[0-9]{3}|tests?/[^\n]*[0-9],[0-9]{3}::테스트 개수::make test'
  '(done|failed|cancelled) [0-9][0-9,]{2,}::GPU 잡 수::make status'
  # 한 자리는 부분집합 서술이라 뺀다. `개` 말고 `항목`·괄호로 닫은 형태도 같은 지표다.
  '(분류 )?규칙 [0-9]{2,}\s?(개|항목|가지)|규칙 [0-9]{2,}\)::분류 규칙 수::make status'
  ...
  # ★ 2026-08-31: `개` 없이 쓴 형태('타이머 7 + 서비스 10')를 놓쳤다.
  #   유닛을 늘렸는데 architecture.md 가 옛 수를 그대로 들고 있었다.
  '타이머 [0-9]+\s?개|타이머 [0-9]+\s*[+·]|서비스 [0-9]+\s?개::유닛 수::systemctl --user list-timers'
  '유닛( 파일)? [0-9]+\s?개::유닛 수::ls systemd/'
  '(상시 )?서비스 [0-9]+\s?개::서비스 수::make status'
  # ★ 2026-08-31 추가. 중복을 지우다 **낡은 수치 다섯 개**가 같이 나왔는데
...
for f in $FILES; do
  ...
  for entry in "${PATTERNS[@]}"; do
    ...
    while IFS=: read -r lineno text; do
      ...
      bad=$((bad+1))
    # ★ 2026-08-31: 굵게 표시가 패턴을 무력화하고 있었다 —
    #   `분류 규칙 **47개**` 는 `규칙 [0-9]{2,}개` 에 안 걸린다. 강조 문자가 사이에 낀다.
    #   패턴을 하나씩 고치면 다음 표기(`__굵게__` · 백틱)에서 또 뚫린다.
    #   **비교 전에 강조 표시를 걷어낸다.** 줄 번호는 sed 가 줄 단위라 그대로 맞는다.
    done < <(sed 's/[*`_]//g' "$f" | grep -nE "$re" 2>/dev/null)
  done
done
